...
class WishlistController extends Controller
{
    ...
    public function index()
    {
        ...
        // Ambil buyer + seluruh relasi wishlist
        $buyer = Buyer::with([
            'wishlistCategories',
            'wishlists.product',          // produk
            'wishlists.wishlistCategory', // kategori wishlist
        ])->find($buyerId);

        // Ambil semua category wishlist (lebih rapi untuk looping)
        $categories = $buyer->wishlistCategories;

        // Ambil seluruh wishlist milik user
        $wishlists = $buyer->wishlists;

        // Untuk menampilkan produk yang ada di wishlist buyer
        $products = Product::with(['carts', 'category', 'photos', 'comments'])
            ->whereHas('wishlists', function ($q) use ($buyerId) {
                $q->where('buyer_id', $buyerId);
            })
            ->paginate(8);

        return view('pages.wishlist.index', compact(
            'categories',
            'wishlists',
            'products'
        ));
    ...
    public function store(Request $request, $product_id)
    {
        ...
        $data = Wishlist::with('wishlistCategory')->where('product_id', $product_id)->get();
        // return $data;
    ...
    public function categoryDetailPage($id)
    {
        ...
        $categoryWishlist = Wishlist::with('wishlistCategory')
            ->where('buyer_id', auth()->user()->buyer->id)
            ->where('wishlist_category_id', $id)
            ->get();
        // return $categoryWishlist;

        $namaCategory = WishlistCategory::where('id', $id)->first();

        return view('pages.wishlist.wishlistCategoryDetail', ['categories' => $categories, 'products' => $products, 'id' => $id, 'categoryWishlist' => $categoryWishlist, 'namaCategory' => $namaCategory]);
    }
